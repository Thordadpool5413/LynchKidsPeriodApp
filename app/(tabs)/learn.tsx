...
export default function LearnScreen() {
  ...
  const items = useMemo(() => EDUCATION_CONTENT.filter((item) => category === 'all' || item.category === category), [category]);

  return (
    <Page>
      <Card tone="butter">
        <Heading size={24}>No weird words. No scary guesses. 🌈</Heading>
        <Body>Short, honest answers written for growing bodies. Medical content is marked draft until clinician review is completed.</Body>
      </Card>
            ...
            <Card tone={item.category === 'safety' ? 'coral' : 'white'}>
              ...
              {open && !locked ? (
                <View style={{ gap: 12, paddingTop: 7 }}>
                  <Body>{item.body}</Body>
                  {item.reviewerStatus === 'draft' ? <Text selectable style={{ color: colors.danger, fontFamily: fonts.utility, fontSize: 12 }}>Draft content — clinical review required before public release.</Text> : null}
