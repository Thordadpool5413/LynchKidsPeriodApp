...
export default function CalendarDayScreen() {
  ...
  return <Page maxWidth={680}>
    ...
    <Card><Text style={{ color: colors.ink, fontFamily: fonts.bodyBold }}>What happened?</Text><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}><ChoiceChip label="Glitter started" selected={kind === 'period-start'} onPress={() => setKind('period-start')} /><ChoiceChip label="Glitter day" selected={kind === 'period-day'} onPress={() => setKind('period-day')} /><ChoiceChip label="Glitter ended" selected={kind === 'period-end'} onPress={() => setKind('period-end')} /><ChoiceChip label="No Glitter today" selected={kind === 'not-on-period'} onPress={() => setKind('not-on-period')} /></View>
