...
export default function ParentScreen() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (existing?.role === 'parent') {
        ...
        if (params.checkout === 'success') setMessage('Payment received. AvaCado is confirming the subscription now.');
        return;
      }
  ...
  if (!session) return <Page maxWidth={720}>
    <Card tone="butter"><GardenGlyph name="lock" size={46} /><Heading size={27}>Grown-up sign in</Heading><Body>AvaCado sends a private, one-time link to your email. No child name, health detail, or request appears in the email.</Body></Card>
  ...
  return <Page maxWidth={1180}>
    ...
    {!dashboard?.linked ? <Card tone="lavender"><Heading size={22}>Finish verified consent</Heading><Body>A legally approved consent provider must verify the parent before a cloud child profile or device link can be created. Production remains safely paused until that provider is enabled.</Body><PrimaryButton label="Start verified consent" onPress={() => Alert.alert('Consent provider required', 'Connect and legally approve the verifiable-parental-consent provider before production activation.')} /></Card> : <>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 14 }}>
        <Card style={{ flexGrow: 1, flexBasis: 300 }}><GardenGlyph name="flower" size={34} /><Heading size={20}>Glitter outlook</Heading><Body>{dashboard?.forecast ? `The next garden moment may be around ${dashboard.forecast.estimatedDate}. Confidence: ${dashboard.forecast.confidence}.` : 'AvaCado needs more tracked Glitter starts before showing an estimate.'}</Body></Card>
      ...
      {dashboard?.careRequests.length ? dashboard.careRequests.map((item) => <Card key={item.id} tone={item.urgentSafety ? 'coral' : item.status === 'open' ? 'butter' : 'white'}><View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 12 }}><Heading size={19}>{item.urgentSafety ? 'Check on them now' : item.status === 'open' ? 'Needs a response' : 'Answered'}</Heading><Body muted>{new Date(item.createdAt).toLocaleString()}</Body></View><Body>{item.urgentSafety ? 'The child tapped the free urgent check-on-me action. Contact them now and follow the fixed safety guidance in AvaCado.' : item.items.join(' · ')}</Body>{item.note ? <><Divider /><Body>“{item.note}”</Body></> : null}{item.status === 'open' ? <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>{(Object.keys(responseLabels) as CareRequestResponseCode[]).map((code) => <SecondaryButton key={code} label={responseLabels[code]} onPress={() => void reply(item, code)} />)}</View> : null}</Card>) : <Card><Body>No garden notes yet.</Body></Card>}
    </>}
    ...
    <Body muted>AvaCado provides supportive information, not diagnosis, telehealth, or emergency monitoring.</Body>
  </Page>;
}
